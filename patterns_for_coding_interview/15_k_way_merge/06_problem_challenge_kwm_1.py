...
# We can go through all the numbers of the two input arrays to create pairs and
# initially insert them all in the heap until we have ‘K’ pairs in Min Heap.
# After that, if a pair is bigger than the top (smallest) pair in the heap, we
# can remove the smallest pair and insert this pair in the heap.

# We can optimize our algorithms in two ways:

# 1. Instead of iterating over all the numbers of both arrays, we can iterate only
#    the first ‘K’ numbers from both arrays. Since the arrays are sorted in descending order,
#    the pairs with the maximum sum will be constituted by the first ‘K’ numbers from both the arrays.
# 2. As soon as we encounter a pair with a sum that is smaller than the smallest (top)
#    element of the heap, we don’t need to process the next elements of the array.
#    Since the arrays are sorted in descending order, we won’t be able to find a pair with
#    a higher sum moving forward.
...
# one element in the heap in each step, the time complexity of the above algorithm will be O(N*M*logK)
# where ‘N’ and ‘M’ are the total number of elements in both arrays, respectively.

# If we assume that both arrays have at least ‘K’ elements then the time complexity can
# be simplified to O(K^2logK), because we are not iterating more than ‘K’ elements in both arrays.
