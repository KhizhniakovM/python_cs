# XOR is a logical bitwise operator that returns 0 (false) if both bits are the same and returns 1 (true)
# otherwise. In other words, it only returns 1 if exactly one bit is set to 1 out of
# the two bits in comparison.

# ../images/13_bitwise_xor/001.png
...
# Time & Space complexity: The time complexity of the above algorithm is O(n)
...
# In other words, XOR of a number with itself will always result in 0.
# This means that if we XOR all the numbers in the input array with all numbers from the range 1
...
def main_2():
    arr = [1, 5, 2, 6, 4]
    print('Missing number is:' + str(find_missing_number_xor(arr)))


main_2()

# Time & Space complexity: The time complexity of the above algorithm is O(n)
# and the space complexity is O(1). The time and space complexities are the same
# as that of the previous solution but, in this algorithm, we will not have any integer overflow problem.

# * MARK: - Important properties of XOR to remember#
