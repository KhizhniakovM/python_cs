...
# we will be left with XOR of num1 and num2 as all other numbers will cancel each other because
# all of them appeared twice. Let’s call this XOR n1xn2. Now that we have XOR of num1 and num2,
# how can we find these two single numbers?

# As we know that num1 and num2 are two different numbers,
# therefore, they should have at least one bit different between them. If a bit in n1xn2 is ‘1’,
# this means that num1 and num2 have different bits in that place, as we know that we can get ‘1’
# only when we do XOR of two different bits, i.e.,

# 1 XOR 0 = 0 XOR 1 = 1
# We can take any bit which is ‘1’ in n1xn2 and partition all numbers
# in the given array into two groups based on that bit. One group will have all
# those numbers with that bit set to ‘0’ and the other with the bit set to ‘1’.
...
# 3. Iterate through all numbers of the input array to partition them into two groups based
#    on rightmostSetBit. Take XOR of all numbers in both the groups separately.
