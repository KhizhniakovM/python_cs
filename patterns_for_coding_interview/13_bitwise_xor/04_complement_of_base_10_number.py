...
# when we change every 1 to a 0 and every 0 to a 1.
...
# 2. It will return 0 if we take XOR of two same bits i.e. 0^0 = 1^1 = 0.
...
# complement will result in a number that has all of its bits set to 1.
# For example, the binary complement of “101” is “010”;
# and if we take XOR of these two numbers, we will get a number with all bits set to 1, i.e.,
# 101 ^ 010 = 111
...
# One way to calculate all_bits_set will be to first count the bits required to store
# the given number. We can then use the fact that for a number which is a complete
# power of ‘2’ i.e., it can be written as pow(2, n), if we subtract ‘1’ from such a number,
