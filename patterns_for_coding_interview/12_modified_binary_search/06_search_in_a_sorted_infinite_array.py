...
# ArrayReader.get(index) will return the number at index; if the array’s size is smaller than the
...
# The only issue with applying binary search in this problem is that we don’t know
# the bounds of the array. To handle this situation, we will first find the proper
# bounds of the array where we can perform a binary search.

# An efficient way to find the proper bounds is to start at the
# beginning of the array with the bound’s size as ‘1’ and exponentially increase
# the bound’s size (i.e., double it) until we find the bounds that can have the key.

# Consider Example-1 mentioned above:
# ../images/12_modified_binary_search/003.png
