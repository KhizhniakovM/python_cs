...
# 1. The array is considered circular, which means if the ‘key’ is bigger than
#    the last letter of the array or if it is smaller than the first letter of the array,
#    the key’s next letter will be the first letter of the array.
# 2. The other difference is that we have to find the next biggest letter which can’t be equal
#    to the ‘key’. This means that we will ignore the case where key == arr[middle].
