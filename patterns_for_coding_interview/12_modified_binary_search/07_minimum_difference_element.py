...
# in a sorted array efficiently, we can use a modified version of the Binary Search
# to find the number that has the minimum difference with the given ‘key’.
...
# return it as the minimum difference number. If we can’t find the ‘key’, (at the end of the loop)
# we can find the differences between the ‘key’ and the numbers pointed out by indices start and end,
# as these two numbers will be closest to the ‘key’. The number that gives minimum difference will
