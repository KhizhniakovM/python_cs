...
# ../images/12_modified_binary_search/004.png

# * MARK: - Example 2
# Input: [4, 5, 7, 9, 10, -1, 2], key = 10
# Output: 4
# Explanation: '10' is present in the array at index '4'.
# ../images/12_modified_binary_search/005.png

# * MARK: - Solution
# The problem follows the Binary Search pattern.
# We can use a similar approach as discussed in Order-agnostic Binary Search and modify
...
#    if the ‘key’ lies between indices start and middle; if it does,
...
# ../images/12_modified_binary_search/006.png

# Since there are no duplicates in the given array, it is always easy to skip one part
# of the array in each iteration. However, if there are duplicates, it is not always
# possible to know which part is sorted. We will look into this case in the ‘Similar Problems’ section.
...
# ../images/12_modified_binary_search/007.png

# Solution
# The only problematic scenario is when the numbers at indices start,
# middle, and end are the same, as in this case, we can’t decide which part of
# the array is sorted. In such a case, the best we can do is to skip one number
# from both ends: start = start + 1 & end = end - 1.
...
# * MARK: - Time Complexity
# This algorithm will run most of the times in O(logN). However, since we only skip
# two numbers in case of duplicates instead of half of the numbers,
