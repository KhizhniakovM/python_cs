...
# Since all the given meetings have merged into one big meeting ([2,5]),
# does this mean that they all are overlapping and we need a minimum of four rooms to hold these meetings?
# You might have already guessed that the answer is NO! As we can clearly see, some meetings are mutually exclusive.
# For example, [2,3] and [3,5] do not overlap and can happen in one room.
...
# 4. If the next meeting m3 is overlapping with m2 we can’t use r1, so we will schedule it in another room
#    (let’s call it r2).
...
#    If the end time of m2 is before the start time of m4, we can use that room r1, otherwise,
#    we need to schedule m4 in another room r3.

# We can conclude that we need to keep track of the ending time of all the meetings currently
# happening so that when we try to schedule a new meeting, we can see what meetings have already ended.
...
# 4. Since the min-heap contains all the active meetings, so before scheduling m1 we can remove all
#    meetings from the heap that have ended before m1, i.e., remove all meetings from the heap that have
...
#    Keep a counter to remember the maximum size of the heap at any time which will be the minimum number of
#    rooms needed.
...
# we’ll have to insert all the meetings into the Min Heap (when all meetings overlap) which will also take O(N)
