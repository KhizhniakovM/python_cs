...
# and we can use a similar dynamic sliding window strategy as discussed
# in Smallest Subarray With a Greater Sum. We can use a HashMap to remember the frequency of
...
# 5. While shrinking, we’ll decrement the character’s frequency going out of the window and remove it
#    from the HashMap if its frequency becomes zero.
...
# in the input string. The outer for loop runs for all characters, and the inner while loop
