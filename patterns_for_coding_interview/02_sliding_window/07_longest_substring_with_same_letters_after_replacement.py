...
# as discussed in Longest Substring with Distinct Characters. We can use a HashMap to count the frequency of
# each letter.
...
# 3. So, at any time, we know that we do have a window with one letter repeating maxRepeatLetterCount times;
#    this means we should try to replace the remaining letters.
#       - If the remaining letters are less than or equal to k, we can replace them all.
#       - If we have more than k remaining letters, we should shrink the window as we cannot replace more
#         than k letters.

# While shrinking the window, we don’t need to update maxRepeatLetterCount
# (hence, it represents the maximum repeating count of ANY letter for ANY window).
# Why don’t we need to update this count when we shrink the window?
# Since we have to replace all the remaining letters to get the longest substring having the same letter
