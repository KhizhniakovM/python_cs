...
# Now, if ‘M’ is positive we will move forward ‘M’ indices and if ‘M’ is negative move backwards
...
# We can start from each index of the array to find the cycle.
...
#    element after the move, we have a one-element cycle.
#    Therefore, we can finish our cycle search for the current element.

# 2. The other requirement mentioned in the problem is that the cycle should not contain
#    both forward and backward movements. We will handle this by remembering the direction
#    of each element while searching for the cycle. If the number is positive,
...
# in the array. This complexity is due to the fact that we are iterating all elements of 
# the array and trying to find a cycle for each element.
...
# If we can remember all the numbers that have been visited, our algorithm will improve to O(N)
# as, then, each number will be evaluated for cycles only once. 
# We can keep track of this by creating a separate array, however, in this case, 
# the space complexity of our algorithm will increase to O(N).