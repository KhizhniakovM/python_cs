...
# number from the range 1 to n based on their creation sequence. This means that the object
...
# Since all numbers are unique, we can try placing each number at its correct place, i.e.,
...
# If we first find a number and then place it at its correct place, it will take us O(N^2),
# which is not acceptable.

# Instead, what if we iterate the array one number at a time, and if the current number we are
# iterating is not at the correct index, we swap it with the number at its correct index.
