...
# Once we have every number in its correct place, we can iterate the array to find the index
# which does not have the correct number, and that index will be our missing number.
...
#    This will result in two changes in our algorithm:
...
#    we can still have the wrong number at index i. This was true in Cyclic Sort too.
#    It didn’t cause any problems in Cyclic Sort as over there,
#    we made sure to place one number at its correct place in each step,
#    but that wouldn’t be enough in this problem as we have one extra number due to the larger range.
#    Therefore, before swapping we will check if the number at index i is within the permissible range i.e.,
#    it is less than the length of the input array, if not, we will skip ahead.
