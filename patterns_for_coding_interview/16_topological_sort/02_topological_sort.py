...
# * MARK: - Solution
# The basic idea behind the topological sort is to provide a partial ordering among
# the vertices of the graph such that if there is an edge from U to V then U≤V i.e.,
# U comes before V in the ordering. Here are a few fundamental concepts related to topological sort:
...
# If the graph has a cycle, some vertices will have cyclic dependencies which makes it impossible to find a
# linear ordering among vertices.
...
# ../images/16_topological_sort/001.png
# ../images/16_topological_sort/002.png
...
def topological_sort(vertices: int, edges: list[list[int]]) -> list[int]:
    ...
    # a. initialize graph
    in_degree: dict[int, int] = {i: 0 for i in range(vertices)}
    graph: dict[int, list[int]] = {i: [] for i in range(vertices)}
    ...
    while sources:
        ...
        for child in graph[vertex]:
            ...
            if in_degree[child] == 0:
                sources.append(in_degree[child])
