...
# * MARK: - Solution
# Since each sequence in the given array defines the ordering of some numbers,
# we need to combine all these ordering rules to find two things:

# 1. Is it possible to construct the originalSeq from all these rules?
# 2. Are these ordering rules not sufficient enough to define the unique ordering of all
#    the numbers in the originalSeq? In other words, can these rules result in more than one sequence?
# Take Example-1:
...
# comes before ‘3’; the third sequence tells us that ‘3’ comes before ‘4’. Combining all
# these sequences will result in a unique sequence: [1, 2, 3, 4].

# The above explanation tells us that we are actually asked to find the topological
# ordering of all the numbers and also to verify that there is only one topological
# ordering of the numbers possible from the given array of the sequences.

# This makes the current problem similar to Tasks Scheduling Order with two differences:

# 1. We need to build the graph of the numbers by comparing each pair of numbers
#    in the given array of sequences.
...
# Since, at most, each pair of numbers can give us one rule, we can conclude that the
# upper bound for the rules is O(N) where ‘N’ is the count of numbers in all sequences.
# So, we can say that the time complexity of our algorithm is O(V+N)

# * MARK: - Space Complexity
# The space complexity will be O(V+N), since we are storing all of the rules
# for each number in an adjacency list.
