...
# 1. Take the first two words “ba” and “bc”. Starting from the beginning of the words,
#    find the first character that is different in both words: it would be ‘a’ from “ba” and
#    ‘c’ from “bc”. Because of the sorted order of words (i.e. the dictionary!), we can
#    conclude that ‘a’ comes before ‘c’ in the alien language.
# 2. Similarly, from “bc” and “ac”, we can conclude that ‘b’ comes before ‘a’.

# These two points tell us that we are actually asked to find the topological ordering
# of the characters, and that the ordering rules should be inferred from adjacent words
# from the alien dictionary.
...
def main():
    print("Character order: " +
          find_order(["a", "b", "ba", "bc", "bca", "bcd", "bcda", "bcde"]))
...
# In step ‘d’, each task can become a source only once and each edge (a rule) will be
...
# where ‘N’ is the number of words in the input. So, we can say that the time
