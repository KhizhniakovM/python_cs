...
# * MARK: - Solution
# This problem follows the Top ‘K’ Elements pattern, and shares similarities
# with Top ‘K’ Frequent Numbers.

# We can use a Max Heap to store the numbers. Instead of comparing the numbers we
# will compare their frequencies so that the root of the heap is always the most
# frequently occurring number. There are two issues that need to be resolved though:

# 1. How can we keep track of the frequencies of numbers in the heap? When we are pushing
#    a new number to the Max Heap, we don’t know how many times the number has already
#    appeared in the Max Heap. To resolve this, we will maintain a HashMap to store the
#    current frequency of each number. Thus whenever we push a new number in the heap,
#    we will increment its frequency in the HashMap and when we pop, we will decrement its frequency.
# 2. If two numbers have the same frequency, we will need to return the number which was
#    pushed later while popping. To resolve this, we need to attach a sequence number to
#    every number to know which number came first.
...
# We will need O(N) space for the heap and the map, so the overall
