...
#    in a Min Heap based on their frequencies. At the same time, we will keep a running
#    count of all the distinct numbers.
...
#    removals we have done.
# 4. If after removing elements from the heap, we are still left with some deletions,
...
# where ‘N’ is the total input numbers. While extracting numbers from the heap,
# in the worst case, we will need to take out ‘K’ numbers. This will happen when we have
# at least ‘K’ numbers with a frequency of two. Since the heap can have a maximum of ‘N/2’
# numbers, therefore, extracting an element from the heap will take O(logN)
# and extracting ‘K’ numbers will take O(KlogN). So overall, the time complexity
# of our algorithm will be O(N*logN + KlogN)
