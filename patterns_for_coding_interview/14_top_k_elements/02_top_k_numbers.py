...
# The time complexity of such an algorithm will be O(N*logN) as we need to use a sorting
# algorithm like Timsort if we use Java’s Collection.sort(). Can we do better than that?
...
# way to repeatedly find the smallest number among a set of numbers will be to use a min-heap.
# As we know, we can find the smallest number in a min-heap in constant time O(1),
# since the smallest number is always at the root of the heap. Extracting the smallest number
# from a min-heap will take O(logN) (if the heap has ‘N’ elements) as the heap needs
# to readjust after the removal of an element.
...
# 2. After the insertion, the heap will have three numbers [3, 1, 5] with ‘1’ being the root
#    as it is the smallest element.
...
#    so let’s take out ‘1’ and insert ‘12’. Now the heap will have [3, 5, 12] with ‘3’ being the
...
# 6. The last number is ‘11’ which is bigger than the root (which is ‘3’),
#    so let’s take out ‘3’ and insert ‘11’. Finally, the heap has the largest three numbers: [5, 12, 11]
...
# first, we insert ‘K’ numbers in the heap and then iterate through the remaining numbers
# and at every step, in the worst case, we need to extract the minimum number and insert a
# new number in the heap. This algorithm is better than O(N*logN)

# Here is the visual representation of our algorithm:
# ../images/14_top_k_elements/001.png
# ../images/14_top_k_elements/002.png
# ../images/14_top_k_elements/003.png
# ../images/14_top_k_elements/004.png
