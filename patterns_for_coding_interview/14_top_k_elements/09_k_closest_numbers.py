...
# in the array. Return the numbers in the sorted order. ‘X’ is not necessarily present in the array.
...
# 1. Since the array is sorted, we can first find the number closest to ‘X’
#    through Binary Search. Let’s say that number is ‘Y’.
# 2. The ‘K’ closest numbers to ‘Y’ will be adjacent to ‘Y’ in the array.
#    We can search in both directions of ‘Y’ to find the closest numbers.
# 3. We can use a heap to efficiently search for the closest numbers. We will
#    take ‘K’ numbers in both directions of ‘Y’ and push them in a Min Heap sorted
#    by their absolute difference from ‘X’. This will ensure that the numbers with
...
# The time complexity of the above algorithm is O(logN + K*logK). We need O(logN)
# for Binary Search and O(K*logK) to insert the numbers in the Min Heap,
...
# * MARK: - Alternate Solution using Two Pointers
...
# a left pointer to move back from ‘Y’ and a right pointer to move forward from ‘Y’.
# At any stage, whichever number pointed out by the left or the right pointer gives the
# smaller difference from ‘X’ will be added to our result list.
...
# whenever we take the number pointed out by the right pointer we will
...
def main_2():
    print("'K' closest numbers to 'X' are: " +
          str(find_closest_elements_alt([5, 6, 7, 8, 9], 3, 7)))
    print("'K' closest numbers to 'X' are: " +
          str(find_closest_elements_alt([2, 4, 5, 6, 9], 3, 6)))
    print("'K' closest numbers to 'X' are: " +
          str(find_closest_elements_alt([2, 4, 5, 6, 9], 3, 10)))


main_2()

# * MARK: - Time Complexity
# The time complexity of the above algorithm is O(logN + K). We need O(logN)
# for Binary Search and O(K) for finding the ‘K’ closest numbers using the two pointers.
