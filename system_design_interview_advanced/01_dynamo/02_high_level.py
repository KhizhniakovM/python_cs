...
# At a high level, Dynamo is a Distributed Hash Table (DHT) that is replicated across the cluster
# for high availability and fault tolerance.

# Introduction: Dynamo’s architecture
# Dynamo’s architecture can be summarized as follows (we will discuss all of these concepts in
# detail in the following lessons):

# Data distribution
# Dynamo uses Consistent Hashing to distribute its data among nodes. Consistent hashing also makes
# it easy to add or remove nodes from a Dynamo cluster.

# Data replication and consistency
# Data is replicated optimistically, i.e., Dynamo provides eventual consistency.

# Handling temporary failures
# To handle temporary failures, Dynamo replicates data to a sloppy quorum of other nodes in
# the system instead of a strict majority quorum.
...
# Since there are no write-time guarantees that nodes agree on values, Dynamo resolves potential
# conflicts using other mechanisms:
