...
# Given a number sequence, find the minimum number of elements that should be deleted to make
...
# * MARK: - Basic Solution
# A basic brute-force solution could be to try deleting all combinations of elements, one by one,
# and checking if that makes the subsequence sorted.

# Alternately, we can convert this problem into a Longest Increasing Subsequence (LIS) problem.
# As we know that LIS will give us the length of the longest increasing subsequence (in the sorted order!),
# which means that the elements which are not part of the LIS should be removed to make the sequence
# sorted. This is exactly what we need. So we’ll get our solution by subtracting the length of LIS
# from the length of the input array: Length-of-input-array - LIS()
