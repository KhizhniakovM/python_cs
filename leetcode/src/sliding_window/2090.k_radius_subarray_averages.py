...
# using integer division. The integer division truncates toward zero, which means
...
class Solution(unittest.TestCase):
    ...
    def getAverages(self, nums, k):
        ...
        if len(nums) < size_to_count:
            return result

        for window_end in range(len(nums)):
            window_sum += nums[window_end]

            if window_end - window_start + 1 < size_to_count:
                result[window_end] = -1
            else:
                center = window_end - k
                result[center] = int(window_sum / size_to_count)
