...
# of a library. Libraries rely on library management systems to manage asset collections as well
# as relationships with their members. Library management systems help libraries keep track of the
...
# ? Librarian:
# Mainly responsible for adding and modifying books, book items, and users. The Librarian can
# also issue, reserve, and return book items.
# ? Member:
...
# BookItem: Any book can have multiple copies, each copy will be considered a book item in our
#   system. Each book item will have a unique barcode.

# Account: We will have two types of accounts in the system, one will be a general member, and
#   the other will be a librarian.

# LibraryCard: Each library user will be issued a library card, which will be used to identify
...
# Rack: Books will be placed on racks. Each rack will be identified by a rack number and will
#   have a location identifier to describe the physical location of the rack in the library.
...
# ? Check-out a book:
# Any library member or librarian can perform this activity. Here are the
# set of steps to check-out a book:

# ../images/011.png

# ? Return a book:
# Any library member or librarian can perform this activity. The system will collect fines
# from members if they return books after the due date. Here are the steps for returning a book:

# ../images/012.png

# ? Renew a book:
# While renewing (re-issuing) a book, the system will check for fines and see if any other member
# has not reserved the same book, in that case the book item cannot be renewed. Here are
...
# Note: This code only focuses on the design part of the use cases. Since you are not required
# to write a fully executable code in an interview, you can assume parts of the code to interact
...
# ? BookItem:
# Encapsulating a book item, this class will be responsible for processing the reservation,
