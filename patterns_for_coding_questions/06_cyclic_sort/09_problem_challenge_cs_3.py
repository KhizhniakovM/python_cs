...
# Find the Smallest Missing Positive Number. The only difference is that, in this problem,
# we need to find the first ‘k’ missing numbers compared to only the first missing number.

# We will follow a similar approach as discussed in Find the Smallest Missing Positive Number
# to place the numbers on their correct indices and ignore all numbers that are
# out of the range of the array. Once we are done with the cyclic sort we will iterate through
# the array to find indices that do not have the correct numbers.

# If we are not able to find ‘k’ missing numbers from the array,
# we need to add additional numbers to the output array.
# To find these additional numbers we will use the length of the array. For example,
# if the length of the array is 4, the next missing numbers will be 4, 5, 6 and so on.
