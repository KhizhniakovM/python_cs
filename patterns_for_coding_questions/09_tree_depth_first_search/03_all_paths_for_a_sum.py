...
# * MARK: - Example
# images/09_tree_depth_first_search/012.png

# * MARK: - Solution
# This problem follows the Binary Tree Path Sum pattern.
# We can follow the same DFS approach. There will be two differences:
...
# and for every leaf node, we might have to store its path (by making a copy of the current path) 
# which will take O(N)

# We can calculate a tighter time complexity of O(NlogN)
# from the space complexity discussion below.

# * MARK: - Space Complexity
# If we ignore the space required for the allPaths list, the space complexity of the above 
# algorithm will be O(N) in the worst case. 
# This space will be used to store the recursion stack. 
# The worst-case will happen when the given tree is a linked list (i.e., every node has only one child).

# How can we estimate the space used for the allPaths array? 
# Take the example of the following balanced tree:

# images/09_tree_depth_first_search/013.png
...
# As we know that there can’t be more than (N+1)/2
...
# As we know that the depth (or height) of a balanced binary tree is O(logN)
