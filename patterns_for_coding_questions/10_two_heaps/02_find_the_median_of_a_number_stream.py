...
# So a brute force solution could be to maintain a sorted list of all numbers inserted in
# the class so that we can efficiently return the median whenever required.
...
# Can we do better than this? Can we utilize the fact that we don’t need the fully sorted list -
# we are only interested in finding the middle element?

# Assume ‘x’ is the median of a list. This means that half of the numbers in the list will be
# smaller than (or equal to) ‘x’ and half will be greater than (or equal to) ‘x’.
# This leads us to an approach where we can divide the list into two halves:
# one half to store all the smaller numbers (let’s call it smallNumList) and one half to store the
# larger numbers (let’s call it largeNumList).
# The median of all the numbers will either be the largest number in the smallNumList or the smallest
# number in the largeNumList. If the total number of elements is even, the median will be
...
# 4. At any time, the median of the current list of numbers can be calculated from the top element
# of the two heaps.
...
class MedianOfAStream:
    ...
    maxHeap: list = []  # containing first half of numbers
    minHeap: list = []  # containing second half of numbers
...
# because, as at any time, we will be storing all the numbers.
