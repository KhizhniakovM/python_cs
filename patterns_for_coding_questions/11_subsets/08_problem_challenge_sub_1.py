...
# find all possible ways in which the expression can be evaluated by grouping the
...
# 4. Once we have the evaluation results from the left and right halves,
...
# The problem has overlapping subproblems, as our recursive calls can be evaluating
# the same sub-expression multiple times. To resolve this, we can use memoization and
# store the intermediate results in a HashMap. In each function call, we can check our map
# to see if we have already evaluated this sub-expression before.
# Here is the memoized version of our algorithm; please see highlighted changes:

def diff_ways_to_evaluate_expression_m(input):
...
def main_2():
    print("Expression evaluations: " +
          str(diff_ways_to_evaluate_expression_m("1+2*3")))

    print("Expression evaluations: " +
          str(diff_ways_to_evaluate_expression_m("2*3-4-5")))
