...
# * MARK: - Solution
# This problem follows the Merge Intervals pattern. Let’s take the above-mentioned example (2)
# and visually draw it:
...
# (e.g., [2,4] and [6,8]), we can calculate a free interval (e.g., [4,6]). This algorithm will take O(N * logN)
# time, where ‘N’ is the total number of intervals. This time is needed because we need to sort all the intervals.
# The space complexity will be O(N), which is needed for sorting. Can we find a better solution?
...
# Once we have the smallest start-time interval, we can then compare it with the next
# smallest start-time interval (again from the Heap) to find the gap.
...
# once (which will take O(N)), and every time we process an interval,
