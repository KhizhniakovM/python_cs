...
def triplet_with_smaller_sum_sim(arr, target):
    arr.sort()
    triplets = []
    for i in range(len(arr)-2):
...
def search_pair_sim(arr, target_sum, first, triplets):
    left = first + 1
    right = len(arr) - 1
    while (left < right):
        if arr[left] + arr[right] < target_sum:  # found the triplet
...
# Another simpler approach could be to check every triplet of the array with three nested loops and
# create a list of triplets that meet the required condition.
