...
# cycle on number ‘1,’ and if the number is not a happy number then the process will be stuck
# in a cycle with a set of numbers. As we saw in Example-2 while determining if ‘12’ is a
# happy number or not, our process will get stuck in a cycle with the following numbers:
# 89 -> 145 -> 42 -> 20 -> 4 -> 16 -> 37 -> 58 -> 89

# We saw in the LinkedList Cycle problem that we can use the Fast & Slow pointers method to
# find a cycle among a set of elements. As we have described above, each number will definitely
# have a cycle. Therefore, we will use the same fast & slow pointer strategy to find the cycle
...
# at most 9^2M, or 81M (this will happen when all digits of ‘N’ are ‘9’).
