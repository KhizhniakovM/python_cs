...
# * MARK: - 1. What is an online movie ticket booking system?
# A movie ticket booking system provides its customers the ability to purchase theatre seats
# online. E-ticketing systems allow the customers to browse through movies currently being
# played and to book seats, anywhere anytime.
...
# 3. Once the user selects a movie, the service should display the cinemas running that movie and
...
# 2. The system will not handle partial ticket orders. Either user gets all the tickets they want or
#   they get nothing.
...
# 5. We can assume that traffic would spike on popular/much-awaited movie releases and the seats would
#   fill up pretty fast. The system should be scalable and highly available to keep up with the surge in traffic.

# * MARK: - 4. Capacity Estimation
# ? Traffic estimates:
# Let’s assume that our service has 3 billion page views per month and sells 10 million tickets a month.

# ? Storage estimates:
# Let’s assume that we have 500 cities and, on average each city has ten cinemas. If there are
# 2000 seats in each cinema and on average, there are two shows every day.
...
# movies and cinemas; let’s assume it’ll take 50 bytes. So, to store all the data about all shows of
# all cinemas of all cities for a day:
...
#   Some allowable values : ‘name,asc’, ‘name,desc’, ‘date,asc’, ‘date,desc’, ‘distance,asc’,
#   ‘name,date,asc’, ‘name,date,desc’, ‘date,name,asc’, ‘date,name,desc’.
...
# 2) “Reservation Failed - Show Full,” 3) “Reservation Failed - Retry, as other users are
# holding reserved seats”.

# * MARK: - 6. Database Design
...
# ../images/system_design_problems/077.png

# * MARK: - 7. High Level Design
# At a high-level, our web servers will manage users’ sessions and application servers will
# handle all the ticket management, storing data in the databases as well as working with the
# cache servers to process reservations.

# ../images/system_design_problems/078.png

# * MARK: - 8. Detailed Component Design
...
# - The seats the user wants to reserve are no longer available, but there are other seats available,
#   so the user is taken back to the theater map to choose different seats.
# - There are no seats available to reserve, but all the seats are not booked yet, as there are
#   some seats that other users are holding in the reservation pool and have not booked yet.
#   The user will be taken to a waiting page where they can wait until the required seats get
#   freed from the reservation pool. This waiting could result in the following options:
#   - If the required number of seats become available, the user is taken to the theater map page
#       where they can choose seats.
#   - While waiting, if all seats get booked or there are fewer seats in the reservation pool
#       than the user intends to book, the user is shown the error message.
...
#   After payment, the booking is marked complete. If the user is not able to pay within five
#   minutes, all their reserved seats are freed to become available to other users.

# ../images/system_design_problems/079.png
# ../images/system_design_problems/080.png
# ../images/system_design_problems/081.png
# ../images/system_design_problems/082.png
# ../images/system_design_problems/083.png
# ../images/system_design_problems/084.png
# ../images/system_design_problems/085.png
# ../images/system_design_problems/086.png

# How would the server keep track of all the active reservations that haven’t been booked yet?
...
# would be keeping track of all the waiting user requests and, as soon as the required number
# of seats become available, it will notify the (the longest waiting) user to choose the seats;
# let’s call it WaitingUserService.

# ? a. ActiveReservationsService
# We can keep all the reservations of a ‘show’ in memory in a data structure similar to Linked
# HashMap or a TreeMap in addition to keeping all the data in the database. We will need a linked
# HashMap kind of data structure that allows us to jump to any reservation to remove it when the
# booking is complete. Also, since we will have expiry time associated with each reservation, the
# head of the HashMap will always point to the oldest reservation record so that the reservation can
# be expired when the timeout is reached.
...
# In the database, we will store the reservation in the ‘Booking’ table and the expiry time will be
# in the Timestamp column. The ‘Status’ field will have a value of ‘Reserved (1)’ and, as soon as a
# booking is complete, the system will update the ‘Status’ to ‘Booked (2)’ and remove the reservation
# record from the Linked HashMap of the relevant show. When the reservation is expired, we can either
# remove it from the Booking table or mark it ‘Expired (3)’ in addition to removing it from memory.

# ActiveReservationsService will also work with the external financial service to process user payments.
# Whenever a booking is completed, or a reservation gets expired, WaitingUsersService will get a
# signal so that any waiting customer can be served.

# ../images/system_design_problems/087.png

# ? b. WaitingUsersService
# Just like ActiveReservationsService, we can keep all the waiting users of a show in memory
# in a Linked HashMap or a TreeMap. We need a data structure similar to Linked HashMap so that we
# can jump to any user to remove them from the HashMap when the user cancels their request. Also,
# since we are serving in a first-come-first-serve manner, the head of the Linked HashMap would
# always be pointing to the longest waiting user, so that whenever seats become available, we can
...
# reservations. As the client will be shown a timer (for the expiration time), which could be a little
# out of sync with the server, we can add a buffer of five seconds on the server to safeguard from a
# broken experience, such that the client never times out after the server, preventing a successful purchase.

# * MARK: - 9. Concurrency
# How to handle concurrency, such that no two users are able to book the same seat. We can use
# transactions in SQL databases to avoid any clashes. For example, if we are using an SQL server
# we can utilize Transaction Isolation Levels to lock the rows before we can update them. Here
# is the sample code:
...
# and Phantoms reads. One thing to note here; within a transaction, if we read rows, we get a write
# lock on them so that they can’t be updated by anyone else.

# Once the above database transaction is successful, we can start tracking the reservation in
# ActiveReservationService.

# * MARK: - 10. Fault Tolerance
# What happens when ActiveReservationsService or WaitingUsersService crashes?
# Whenever ActiveReservationsService crashes, we can read all the active reservations from the
# ‘Booking’ table. Remember that we keep the ‘Status’ column as ‘Reserved (1)’ until a reservation
# gets booked. Another option is to have a primary-secondary configuration so that, when the
# primary crashes, the secondary can take over. We are not storing the waiting users in the
# database, so, when WaitingUsersService crashes, we don’t have any means to recover that data
# unless we have a primary-secondary setup.

# Similarly, we’ll have a primary-secondary setup for databases to make them fault-tolerant.

# * MARK: - 11. Data Partitioning
# Database partitioning: If we partition by ‘MovieID’, then all the Shows of a movie will be
# on a single server. For a very hot movie, this could cause a lot of load on that server. A better
# approach would be to partition based on ShowID; this way, the load gets distributed among different servers.

# ActiveReservationService and WaitingUserService partitioning: Our web servers will manage all
# the active users’ sessions and handle all the communication with the users. We can use the
...
# particular show will be handled by a certain set of servers. Let’s assume for load balancing
# our Consistent Hashing allocates three servers for any Show, so whenever a reservation is expired,
# the server holding that reservation will do the following things:

# 1. Update the database to remove the Booking (or mark it expired) and update the seats’ Status
#   in ‘Show_Seats’ table.
...
# 1. The server holding that reservation sends a message to all servers holding the waiting users
#   of that Show, so that those servers can expire all the waiting users that need more seats
