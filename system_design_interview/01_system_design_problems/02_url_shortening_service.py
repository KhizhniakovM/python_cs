...
# * MARK: - 1. Why do we need URL shortening?
...
# URL shortening is used to optimize links across devices, track individual links to analyze audience,
# measure ad campaigns’ performance, or hide affiliated original URLs.

# If you haven’t used tinyurl.com before, please try creating a new shortened URL and spend
# some time going through the various options their service offers.
...
# compared to new URL shortenings. Let’s assume a 100:1 ratio between read and write.
...
# ../images/system_design_problems/002.png
...
# ? Memory estimates:
# If we want to cache some of the hot URLs that are frequently accessed,
# how much memory will we need to store them? If we follow the 80-20 rule,
# meaning 20% of URLs generate 80% of traffic, we would like to cache these 20% hot URLs.
...
# One thing to note here is that since there will be many duplicate requests (of the same URL),
...
# ../images/system_design_problems/003.png
...
# Each api_dev_key can be limited to a certain number of URL creations and redirections per some
# time period (which may be set to a different duration per developer key).

# * MARK: - 5. Database Design
# 💡 Defining the DB schema in the early stages of the interview would help to understand
# the data flow among various components and later would guide towards data partitioning.
...
# one for storing information about the URL mappings and
# one for the user’s data who created the short link.

# ../images/system_design_problems/004.png

# ? What kind of database should we use?
# Since we anticipate storing billions of rows,
# and we don’t need to use relationships between objects – a NoSQL store like DynamoDB,
# Cassandra or Riak is a better choice. A NoSQL choice would also be easier to scale.
# Please see SQL vs NoSQL for more details.

# * MARK: - 6. Basic System Design and Algorithm
# The problem we are solving here is how to generate a short and unique key for a given URL.

# In the TinyURL example in Section 1, the shortened URL is “https://tinyurl.com/rxcsyr3r”.
# The last eight characters of this URL constitute the short key we want to generate.
# We’ll explore two solutions here:

# ? a. Encoding actual URL
# We can compute a unique hash (e.g., MD5 or SHA256, etc.) of the given URL.
# The hash can then be encoded for display. This encoding could be base36 ([a-z ,0-9])
# or base62 ([A-Z, a-z, 0-9]) and if we add ‘+’ and ‘/’ we can use Base64 encoding.
...
# Now we only have space for 6 (or 8) characters per short key;
# how will we choose our key then? We can take the first 6 (or 8) letters for the key.
# This could result in key duplication; to resolve that, we can choose some other characters
# out of the encoding string or swap some characters.
...
# 2. What if parts of the URL are URL-encoded? e.g., http://www.educative.io/distributed.php?id=design,
...
# We can append an increasing sequence number to each input URL
# to make it unique and then generate its hash. We don’t need to store this sequence number
# in the databases, though. Possible problems with this approach could be an ever-increasing
# sequence number. Can it overflow? Appending an increasing sequence number will also impact
...
# ../images/system_design_problems/004.png
# ../images/system_design_problems/005.png
# ../images/system_design_problems/006.png
# ../images/system_design_problems/007.png
# ../images/system_design_problems/008.png
# ../images/system_design_problems/009.png
# ../images/system_design_problems/010.png
# ../images/system_design_problems/011.png
# ../images/system_design_problems/012.png
# ../images/system_design_problems/013.png

# ? b. Generating keys offline
# We can have a standalone Key Generation Service (KGS) that generates random six-letter
# strings beforehand and stores them in a database (let’s call it key-DB).
# Whenever we want to shorten a URL, we will take one of the already-generated keys and use it.
# This approach will make things quite simple and fast. Not only are we not encoding the URL,
...
# Servers can use KGS to read/mark keys in the database. KGS can use two tables to store keys:
...
# This ensures each server gets unique keys. If KGS dies before assigning all the loaded keys
# to some server, we will be wasting those keys–which could be acceptable,
# given the huge number of keys we have.

# KGS also has to make sure not to give the same key to multiple servers.
# For that, it must synchronize (or get a lock on) the data structure holding the keys before
...
# We can look up the key in our database to get the full URL. If it’s present in the DB,
# issue an “HTTP 302 Redirect” status back to the browser, passing the stored URL in the “Location”
# field of the request. If that key is not present in our system,
# issue an “HTTP 404 Not Found” status or redirect the user back to the homepage.

# ? Should we impose size limits on custom aliases?
# Our service supports custom aliases. Users can pick any ‘key’ they like,
# but providing a custom alias is not mandatory. However, it is reasonable
# (and often desirable) to impose a size limit on a custom alias to ensure we have a consistent
# URL database. Let’s assume users can specify a maximum of 16 characters per customer key
# (as reflected in the above database schema).

# ../images/system_design_problems/014.png

# * MARK: - 7. Data Partitioning and Replication
# To scale out our DB, we need to partition it so that it can store information about billions of URLs.
# Therefore, we need to develop a partitioning scheme that would divide and store our
# data into different DB servers.

# ? a. Range Based Partitioning:
# We can store URLs in separate partitions based on the hash key’s first letter.
# Hence we will save all the URL hash keys starting with the letter ‘A’ (and ‘a’) in one partition,
# save those that start with the letter ‘B’ in another partition, and so on.
...
# For example, we decide to put all URLs starting with the letter ‘E’ into a DB partition,
# but later we realize that we have too many URLs that start with the letter ‘E.’

# ? b. Hash-Based Partitioning:
# In this scheme, we take a hash of the object we are storing.
# We then calculate which partition to use based upon the hash. In our case,
# we can take the hash of the ‘key’ or the short link to determine the partition in
# which we store the data object.

# Our hashing function will randomly distribute URLs into different partitions
# (e.g., our hashing function can always map any ‘key’ to a number between [1…256]).
...
# We can use any off-the-shelf solution like Memcached,
# which can store full URLs with their respective hashes. Thus, the application servers,
# before hitting the backend storage, can quickly check if the cache has the desired URL.

# ? How much cache memory should we have?
# We can start with 20% of daily traffic and, based on clients’ usage patterns,
# we can adjust how many cache servers we need. As estimated above, we need 170GB of memory
# to cache 20% of daily traffic. Since a modern-day server can have 256GB of memory,
# we can easily fit all the cache into one machine. Alternatively, we can use a couple of
# smaller servers to store all these hot URLs.
...
# We can use a Linked Hash Map or a similar data structure to store our URLs and Hashes,
# which will also keep track of the URLs that have been accessed recently.
...
# ../images/system_design_problems/015.png
# ../images/system_design_problems/016.png
# ../images/system_design_problems/017.png
# ../images/system_design_problems/018.png
# ../images/system_design_problems/019.png
# ../images/system_design_problems/020.png
# ../images/system_design_problems/021.png
# ../images/system_design_problems/022.png
# ../images/system_design_problems/023.png
# ../images/system_design_problems/024.png
# ../images/system_design_problems/025.png

# * MARK: - 9. Load Balancer (LB)
...
# requests equally among backend servers. This LB is simple to implement and does not
# introduce any overhead. Another benefit of this approach is that if a server is dead,
# LB will take it out of the rotation and stop sending any traffic to it.
...
# To handle this, a more intelligent LB solution can be placed that periodically
# queries the backend server about its load and adjusts traffic based on that.

# * MARK: - 10. Purging or DB cleanup
...
# If we chose to continuously search for expired links to remove them,
# it would put a lot of pressure on our database. Instead, we can slowly remove expired
# links and do a lazy cleanup. Our service will ensure that only expired links will be deleted,
# although some expired links can live longer but will never be returned to users.

# - Whenever a user tries to access an expired link, we can delete the link and return an error to the user.
# - A separate Cleanup service can run periodically to remove expired links from our storage and cache.
#   This service should be very lightweight and scheduled to run only when the user traffic is
#   expected to be low.
...
# we can send an error (HTTP 401) back. Given that we are storing our data in a NoSQL
# wide-column database like Cassandra, the key for the table storing permissions would be
# the ‘Hash’ (or the KGS generated ‘key’). The columns will store the UserIDs of those users
# that have permission to see the URL.
