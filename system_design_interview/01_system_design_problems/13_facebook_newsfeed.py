...
# * MARK: - 1. What is Facebook’s newsfeed?
# A Newsfeed is the constantly updating list of stories in the middle of Facebook’s homepage.
# It includes status updates, photos, videos, links, app activity, and ‘likes’ from people,
# pages, and groups that a user follows on Facebook. In other words, it is a compilation of a
# complete scrollable version of your friends’ and your life story from photos, videos, locations,
# status updates, and other activities.
...
# 1. Our system should be able to generate any user’s newsfeed in real-time - maximum latency seen by
#   the end user would be 2s.
...
# ? Traffic estimates:
# Let’s assume 300M daily active users with each user fetching their timeline an average of five
# times a day. This will result in 1.5B newsfeed requests per day or approximately 17,500 requests per second.

# ? Storage estimates:
# On average, let’s assume we need to have around 500 posts in every user’s feed that we want
# to keep in memory for a quick fetch. Let’s also assume that on average each post would be 1KB
# in size. This would mean that we need to store roughly 500KB of data per user. To store all this
...
# - since_id (number): Optional; returns results with an ID higher than (that is, more recent than)
#   the specified ID.
...
# There are three primary objects: User, Entity (e.g. page, group, etc.), and FeedItem (or Post). Here
# are some observations about the relationships between these entities:
...
# entities, we can store this relation in a separate table. The “Type” column in “UserFollow” identifies
# if the entity being followed is a User or Entity. Similarly, we can have a table for FeedMedia relation.

# ../images/system_design_problems/071.png

# * MARK: - 6. High Level System Design
# At a high level this problem can be divided into two parts:

# ? Feed generation:
# Newsfeed is generated from the posts (or feed items) of users and entities (pages and groups)
# that a user follows. So, whenever our system receives a request to generate the feed for a
...
# steps to rank and add the newer posts to her feed. Jane can then be notified that there are newer
# items in her feed that she can fetch.

# ? Feed publishing:
# Whenever Jane loads her newsfeed page, she has to request and pull feed items from the server.
# When she reaches the end of her current feed, she can pull more data from the server. For newer
# items either the server can notify Jane and then she can pull, or the server can push, these new
...
# 2. Application server: To execute the workflows of storing new posts in the database servers. We will
#   also need some application servers to retrieve and to push the newsfeed to the end user.
...
#   newsfeed and store in the cache. This service will also receive live updates and will add these newer
#   feed items to any user’s timeline.
...
# ../images/system_design_problems/072.png

# * MARK: - 7. Detailed Component Design
# Let’s discuss different components of our system in detail.

# ? a. Feed generation
# Let’s take the simple case of the newsfeed generation service fetching most recent posts from
# all the users and entities that Jane follows; the query would look like this:
...
# We can have dedicated servers that are continuously generating users’ newsfeed and storing them
# in memory. So, whenever a user requests for the new posts for their feed, we can simply serve it
# from the pre-generated, stored location. Using this scheme, user’s newsfeed is not compiled on
# load, but rather on a regular basis and returned to users whenever they request for it.

# Whenever these servers need to generate the feed for a user, they will first query to see what was
...
# us to not only jump to any feed item but also iterate through the map easily. Whenever users want
# to fetch more feed items, they can send the last FeedItemID they currently see in their newsfeed,
...
# Initially, we can decide to store 500 feed items per user, but this number can be adjusted
# later based on the usage pattern. For example, if we assume that one page of a user’s feed has
# 20 posts and most of the users never browse more than ten pages of their feed, we can decide to
# store only 200 posts per user. For any user who wants to see more posts (more than what is stored in memory),
# we can always query backend servers.

# ? Should we generate (and keep in memory) newsfeeds for all users?
# There will be a lot of users that don’t log-in frequently. Here are a few things we can do to
# handle this; 1) a more straightforward approach could be, to use an LRU based cache that can
# remove users from memory that haven’t accessed their newsfeed for a long time 2) a smarter
# solution can figure out the login pattern of users to pre-generate their newsfeed, e.g., at
...
# ? b. Feed publishing
# The process of pushing a post to all the followers is called fanout. By analogy, the push
...
# so that users can pull it from the server whenever they need it. Clients can pull the feed data
# on a regular basis or manually whenever they need it. Possible problems with this approach are
# a) New data might not be shown to the users until they issue a pull request, b) It’s hard to find
# the right pull cadence, as most of the time pull requests will result in an empty response if there
# is no new data, causing waste of resources.

# ? "Push" model or Fan-out-on-write:
# For a push system, once a user has published a post, we can immediately push this post to all the
# followers. The advantage is that when fetching feed you don’t need to go through your friend’s
# list and get feeds for each of them. It significantly reduces read operations. To efficiently
# handle this, users have to maintain a Long Poll request with the server for receiving the updates.
# A possible problem with this approach is that when a user has millions of followers (a celebrity-user)
# the server has to push updates to a lot of people.

# ? Hybrid:
# An alternate method to handle feed data could be to use a hybrid approach, i.e., to do a
...
# or followers, by disabling fanout for them, we can save a huge number of resources. Another alternate
# approach could be that, once a user publishes a post, we can limit the fanout to only her online
# friends. Also, to get benefits from both the approaches, a combination of ‘push to notify’ and
# ‘pull for serving’ end-users is a great way to go. Purely a push or pull model is less versatile.
...
# But, we should let the client specify how many feed items they want with each request as the user
# may like to fetch a different number of posts depending on the device (mobile vs. desktop).

# ? Should we always notify users if there are new posts available for their newsfeed?
# It could be useful for users to get notified whenever new data is available. However, on mobile devices,
# where data usage is relatively expensive, it can consume unnecessary bandwidth. Hence, at least
...
# but today’s ranking algorithms are doing a lot more than that to ensure “important” posts are
# ranked higher. The high-level idea of ranking is first to select key “signals” that make a post
# important and then to find out how to combine them to calculate a final ranking score.

# More specifically, we can select features that are relevant to the importance of any feed item, e.g.,
# number of likes, comments, shares, time of the update, whether the post has images/videos, etc.,
# and then, a score can be calculated using these features. This is generally enough for a simple
# ranking system. A better ranking system can significantly improve itself by constantly evaluating
# if we are making progress in user stickiness, retention, ads revenue, etc.

# * MARK: - 9. Data Partitioning#
...
# distribute our data onto multiple machines such that we can read/write it efficiently. For sharding
...
# function that will map the user to a cache server where we will store the user’s feed objects.
# Also, for any given user, since we don’t expect to store more than 500 FeedItemIDs, we will not
# run into a scenario where feed data for a user doesn’t fit on a single server. To get the feed of
# a user, we would always have to query only one server. For future growth and replication, we must use
# Consistent Hashing.
