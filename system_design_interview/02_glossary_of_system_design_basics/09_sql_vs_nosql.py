...
# Both of them differ in the way they were built, the kind of information they store,
# and the storage method they use.

# Relational databases are structured and have predefined schemas like phone books
# that store phone numbers and addresses. Non-relational databases are unstructured,
# distributed, and have a dynamic schema like file folders that hold everything from a
# person’s address and phone number to their Facebook ‘likes’ and online shopping preferences.
...
# Some of the most popular relational databases are MySQL, Oracle, MS SQL Server, SQLite,
...
# ? Key-Value Stores:
# Data is stored in an array of key-value pairs. The ‘key’ is an attribute name which
...
# and these documents are grouped together in collections. Each document can have an entirely
# different structure. Document databases include the CouchDB and MongoDB.

# ? Wide-Column Databases:
# Instead of ‘tables,’ in columnar databases we have column families,
# which are containers for rows. Unlike relational databases, we don’t need to know
# all the columns up front and each row doesn’t have to have the same number of columns.
...
# * MARK: - High level differences between SQL and NoSQL
# ? Storage:
# SQL stores data in tables where each row represents an entity and each column
# represents a data point about that entity; for example, if we are storing a car
...
# ? Schema:
# In SQL, each record conforms to a fixed schema, meaning the columns must be decided
# and chosen before data entry and each row must have data for each column.
...
# Sometimes it is also called UnQL (Unstructured Query Language). Different databases
# have different syntax for using UnQL.

# ? Scalability:
# In most common situations, SQL databases are vertically scalable, i.e., by increasing
# the horsepower (higher Memory, CPU, etc.) of the hardware, which can get very expensive.
...
# Any cheap commodity hardware or cloud instances can host NoSQL databases, thus making
# it a lot more cost-effective than vertical scaling. A lot of NoSQL technologies also
# distribute data across servers automatically.

# ? Reliability or ACID Compliancy (Atomicity, Consistency, Isolation, Durability):
# The vast majority of relational databases are ACID compliant. So, when it comes to data
# reliability and safe guarantee of performing transactions, SQL databases are still the better bet.

# Most of the NoSQL solutions sacrifice ACID compliance for performance and scalability.

# * MARK: - SQL VS. NoSQL - Which one to use?
...
#    for many e-commerce and financial applications, an ACID-compliant database remains the preferred option.
# 2. Your data is structured and unchanging. If your business is not experiencing massive growth
#    that would require more servers and if you’re only working with data that is consistent,
#    then there may be no reason to use a system designed to support a variety of data types
#    and high traffic volume.

# ? Reasons to use NoSQL database
# When all the other components of our application are fast and seamless,
# NoSQL databases prevent data from being the bottleneck. Big data is contributing to a
# large success for NoSQL databases, mainly because it handles data differently than
# the traditional relational databases. A few popular examples of NoSQL databases are MongoDB,
# CouchDB, Cassandra, and HBase.

# 1. Storing large volumes of data that often have little to no structure. A NoSQL database
#    sets no limits on the types of data we can store together and allows us to add new types
#    as the need changes. With document-based databases, you can store data in one place without
...
#    cost-saving solution but requires data to be easily spread across multiple servers to scale up.
#    Using commodity (affordable, smaller) hardware on-site or in the cloud saves you the hassle
#    of additional software and NoSQL databases like Cassandra are designed to be scaled across
#    multiple data centers out of the box, without a lot of headaches.
# 3. Rapid development. NoSQL is extremely useful for rapid development as it doesn’t need to be
#    prepped ahead of time. If you’re working on quick iterations of your system which require
