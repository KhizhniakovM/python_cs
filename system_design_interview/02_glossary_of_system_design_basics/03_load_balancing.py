...
# requests across multiple servers, a load balancer reduces individual server load
...
# ../images/glossary/003.png

# * MARK: - Benefits of Load Balancing
...
#   failure won’t affect the end user experience as the load balancer will simply route around
...
# They will first ensure that the server they choose is actually responding appropriately
# to requests and then use a pre-configured algorithm to select one from the set of healthy servers.
# We will discuss these algorithms shortly.

# ? Health Checks
# Load balancers should only forward traffic to “healthy” backend servers.
# To monitor the health of a backend server, “health checks” regularly attempt to
# connect to backend servers to ensure that servers are listening. If a server fails a health check,
# it is automatically removed from the pool, and traffic will not be forwarded to it until it
# responds to the health checks again.

# There is a variety of load balancing methods, which use different algorithms for different needs.

# - Least Connection Method — This method directs traffic to the server with the fewest active connections.
#   This approach is quite useful when there are a large number of persistent client connections which
#   are unevenly distributed between the servers.
...
#   request to the next server. When it reaches the end of the list, it starts over at the beginning.
...
#   (an integer value that indicates the processing capacity).
...
# Each LB monitors the health of the other and, since both of them are equally
# capable of serving traffic and failure detection, in the event the main load balancer
# fails, the second load balancer takes over.

# ../images/glossary/004.png
